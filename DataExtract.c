...
int main(){
    ...
    fid1 = fopen("data.txt","r");        // data from the OBC
    fid2 = fopen("0x10_Gallassia_reconstruct.txt","w");
    fid3 = fopen("0x10_Gallassia_final.csv","w");
...
void ReConstructData(unsigned char* framebytes){
    ...
    APDcounts[6] = (temp32MSB + temp32LSB) & 0x00000FFF; // APD2 dac,12bits [1-7]
    ...
    temp = Reconsdata[10];
    Thermistors[4] = temp & 0x00FF;     // 5x thermistors [8-12]
    
    temp32LSB = Reconsdata[10];
    temp32LSB = temp32LSB >> 8;
    temp32MSB = Reconsdata[11];
    temp32MSB = temp32MSB << 8;
    LaserDAC = (temp32MSB + temp32LSB) & 0x0FFF; // laser DAC, 12bits [13]
    ...
    LCPR_Parameters[0] = (temp32MSB + temp32LSB) & 0x0FFF; // [14]
